...
echo "[2b] plain cd into the review worktree sees a real git checkout, no env needed"
[ -e "$WT/.git" ] || fail "review worktree has no .git — not a real linked worktree"
plain_status="$(cd "$WT" && git status --porcelain)"
assert_eq "$plain_status" "" "plain git status is clean outside the review shell"
plain_branch="$(cd "$WT" && git rev-parse --abbrev-ref HEAD)"
assert_eq "$plain_branch" "feature" "plain git sees the target branch checked out"
plain_log="$(cd "$WT" && git log --oneline -1 --format=%s)"
assert_eq "$plain_log" "feat" "plain git log shows the target branch's own commit"
plain_remote="$(cd "$WT" && git remote get-url origin)"
assert_eq "$plain_remote" "$ORIGIN" "plain git sees the project's real remote"
proj_worktrees="$(cd "$TMP/proj" && git worktree list --porcelain)"
assert_contains "$proj_worktrees" "$WT" "the project's own git worktree list includes the review checkout"
